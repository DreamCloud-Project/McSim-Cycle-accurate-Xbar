...
#include "commons/mapping_heuristic/dcMappingHeuristicStaticTriCore.hxx"
...
#include "xbar/xbar.hxx"
#include "xbar/lib/xbar_sc_fifo.h"
...
//      USING     //
////////////////////
using dreamcloud::platform_sclib::xbar::XBAR;
...
public:
	...
	dcSystem(sc_module_name name, dcSimuParams params_) :
			sc_module(name), params(params_), pktsFromPe("pktFromPe",
					...
					0), nbPacketSendInNoc(0), nbpacketReceivedFromNoc(0), sig_inp(
					"xbInput"), sig_out("xbOutput") {

		bufferSize = params.getXbarBuffSize();
		sig_inp.init(params.getDimension(), create_fifo);
		sig_out.init(params.getDimension(), create_fifo);
...
// Open files
		...
		nocTrafficCsvFile.open(
				params.getOutputFolder() + "/OUTPUT_NoC_Traces.csv");
		nocTrafficCsvFile
		...
		cout
				...
				<< endl;
		cout
				<< "**** Summary of platform (cores + XBAR) simulation results ****"
				<< endl;
		cout
				<< "***************************************************************"
				<< endl;
		...
		if (params.getMappingHeuristic() == "Static"
				...
				|| params.getMappingHeuristic() == "StaticTriCore") {
...
//		cout << "    XBAR buffer size                    : " << params.getXbarBuffSize() << endl;
//		cout << "    XBAR arbitration policy             : " << params.getXbarPolicy() << endl;
//		cout << "    XBAR local read cost                : " << params.getLocalReadCost() << endl;
//		cout << "    XBAR remote read cost               : " << params.getRemoteReadCost() << endl;
//		cout << "    XBAR local write cost               : " << params.getLocalWriteCost() << endl;
//		cout << "    XBAR remote write cost              : " << params.getRemoteWriteCost() << endl;
		cout << endl << endl;
...
//				}
		...
		} else if (params.getMappingHeuristic() == "StaticTriCore") {
			dcMappingHeuristicStaticTriCore *staticMappingTriCore =
					new dcMappingHeuristicStaticTriCore();
			staticMappingTriCore->setFile(params.getMappingFile());
			mappingHeuristic = staticMappingTriCore;
...
//				<< application->GetAllInstructions(taskGraph).size() << endl;
		...
		for (unsigned int row(0); row < params.getRows(); ++row) {
			for (unsigned int col(0); col < params.getCols(); ++col) {
				...
				pes[row][col]->pe_inputs(sig_inp.at(counter));
				pes[row][col]->pe_outputs(sig_out.at(counter));

				counter++;
			}
		}

		// Creating the XBAR interconnect
		xbar = new XBAR("XBAR", params_);
		xbar->clk(clock);

		xbar->xb_inputs(sig_out);
		xbar->xb_outputs(sig_inp);
...
private:
	...
	//XBAR
	XBAR* xbar = NULL;
	/*sc_fifo<Packet> sig_inp0;
	 sc_fifo<Packet> sig_inp1;
	 sc_fifo<Packet> sig_inp2;
	 sc_fifo<Packet> sig_inp3;

	 sc_fifo<Packet> sig_out0;
	 sc_fifo<Packet> sig_out1;
	 sc_fifo<Packet> sig_out2;
	 sc_fifo<Packet> sig_out3;*/

	sc_vector<xb_sc_fifo<Packet>> sig_inp;
	sc_vector<xb_sc_fifo<Packet>> sig_out;
	...
	// Utility functions
	void dumpNoCLoadGraphFile();
	void dumpTraces();
	void dumpTaskGraphFile();
	void dumpTaskAndRunnableGraphFile();
	...
	static xb_sc_fifo<Packet>*
	create_fifo(const char* nm, size_t) {
		return new xb_sc_fifo<Packet>(nm, bufferSize);
	}
};
